...
| 7.5 | Create OG image for LinkedIn preview | ✅ Done | Implemented as `src/app/opengraph-image.tsx` using Next.js's built-in `next/og` (`ImageResponse`) instead of a static PNG — generates a real 1200x630 image server-side from `personalInfo` in `content.ts`, matching Hero's dark slate-950 + indigo/violet theme. Next.js auto-injects `og:image` and `twitter:image` meta tags (verified in rendered HTML). Added `metadataBase` to `layout.tsx` so image URLs resolve absolute (required by LinkedIn). No caching added on the route (regenerates per request) — accepted as fine given low traffic. |
| 7.6 | Add Vercel Analytics (optional) | ⬜ Todo | Free tier available. Deferred for now per Anuj — skipped to 7.7, can revisit later |
| 7.7 | Update LinkedIn profile with live URL | ✅ Done | Anuj updated LinkedIn (Contact Info / Featured) with `https://anujmittal.site` |
...
| 2026-07-01 | Phase 7 tasks 7.1–7.4 done: repo connected to Vercel, `DEEPSEEK_API_KEY` added (scoped to Production and Preview), deploy verified live, and a global daily message cap (100/day, in-memory, date-based reset) added in `chat/route.ts` alongside the existing 20/IP limit — committed and pushed (`1486d21`). Live site confirmed working post-deploy. Anuj asked about adding a custom domain — walked through the steps (buy domain → Vercel Settings → Domains → add DNS records at registrar → wait for propagation) but nothing set up yet; no domain purchased/added as of session end. | Resume: decide on custom domain (register one? which name?) and whether to add it as a formal task; then task 7.5 (create OG image for LinkedIn preview, 1200x630px) |
| 2026-07-02 | Custom domain purchased: `anujmittal.site` (Namecheap). Added domain in Vercel, switched Namecheap to Custom DNS with Vercel's nameservers (`ns1`/`ns2.vercel-dns.com`) — confirmed correctly saved via whois, DNS propagation in progress at session end (Vercel still showed "invalid configuration" pending propagation). Task 7.5 done: built `src/app/opengraph-image.tsx` using Next.js's built-in `next/og` (dynamic 1200x630 image generation from `personalInfo`, styled to match Hero's dark indigo/violet theme) instead of a static PNG, and added `metadataBase: "https://anujmittal.site"` to `layout.tsx`. Verified via build, lint, rendered meta tags (`og:image`, `twitter:image`), and actual PNG dimensions (1200x630 confirmed). No route caching added — accepted given low traffic. Committed and pushed. | Confirm domain propagation finished and live site loads at `anujmittal.site`; then tasks 7.6 (Vercel Analytics, optional), 7.7 (update LinkedIn with live URL — now the custom domain), 7.8 (Lighthouse audit) |
| 2026-07-02 | Domain propagation confirmed complete — verified `https://anujmittal.site` returns HTTP 200, served by Vercel, correct page title, SSL issued. Task 7.6 (Vercel Analytics) deferred — Anuj chose to skip it for now (optional, can revisit). Task 7.7 done — Anuj updated LinkedIn Contact Info / Featured section with the live `https://anujmittal.site` URL. Session paused here, no code changes this session (no commit needed). | Resume: task 7.8 (final Lighthouse audit on `https://anujmittal.site`, target >90 on all metrics), and optionally revisit 7.6 (Vercel Analytics) |